...
async function getDashboardData(userId: string) {
  ...
  return {
    ...
    totalInvested: investments.reduce((sum: number, i: any) => sum + Number(i.currentValue), 0),
...
export default async function DashboardPage() {
  ...
  const quickLinks: Array<[string, string]> = [
    ["💸 Adicionar gasto", "/expenses"],
    ["🎯 Criar meta", "/goals"],
    ["🤖 Conversar com IA", "/ai-chat"],
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,hsl(252 50% 12%),hsl(234 24% 11%))", border: `1px solid hsl(252 50% 22%)`, borderRadius: "12px", padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          {quickLinks.map(([btn, href]: [string, string]) => (
